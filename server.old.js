...
app.get('/api/users', (req, res) => {
    ...
    con.query('SELECT * FROM users where id != "' + param + '" ',  (error, results, fields) =>{
...
app.post('/api/register',  (req, res) => {
    ...
    con.query("SELECT id from users WHERE email=?  LIMIT 1", [email], (err, rows, fields) => {
        if (rows.length != 0) {
            ...
            con.query("INSERT INTO users (firstname, lastname,email,password,created_at) VALUES" +
                "('" + firstname + "', '" + lastname + "','" + email + "','" + pass + "','" + createdTime + "')", (err, result) => {
                    if (err) throw err;
...
app.post('/api/login', (req, res) => {
    const email = req.body.email;
    const pass = req.body.password;
    con.query("SELECT id,firstname,lastname from users WHERE email=? and password=? Limit 1", [email, pass], (err, rows, fields) => {
        if (rows.length != 0) {
            data["status"] = "OK";
...
app.post('/api/validateEmail',  (req, res) => {
    const email = req.body.email;
    con.query("SELECT id from users WHERE email=?", [email],  (err, rows, fields) => {
        if (rows.length != 0) {
            data["status"] = "ERR";
...
app.post('/api/getUserCustomMenus', (req, res) => {
    ...
    con.query('SELECT menuname,path,created_at,menu_display_order FROM users_custom_menus WHERE user_id=? ', [userID], (error, rows, fields) => {
        if (error) throw error;
        if (rows.length != 0) {
            data["status"] = "OK";
...
app.post('/api/getUserTodoLists', (req, res) => {
    const userID = req.body.user;
    con.query('SELECT * FROM users_todo_lists WHERE user_id=? ', [userID], (error, rows, fields) => {
        if (error) throw error;
        if (rows.length != 0) {
            data["status"] = "OK";
...
app.post('/api/updateTodoStatus', (req, res) => {
    ...
    const sql = "UPDATE users_todo_lists SET completed_at = '" + completed_at + "',todo_status = '" + todoStatus + "' WHERE id= '" + taskRowId + "'";
...
app.post('/api/sendMessage', (req, res) =>{
    ...
    //console.log(values);
    const sql = "INSERT INTO users_msg  (senduser_id, receiveduser_id,send_msg,created_at,msg_status,reply_msg) VALUES ?";
...
app.get('/api/userMessages', (req, res) => {
    ...
    const sql = 'SELECT * FROM users_msg where ' + queryCondition + ' ORDER BY id ' + orderWise + '';
